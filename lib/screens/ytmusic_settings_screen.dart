...
class YTMusicSettingsScreen extends ConsumerStatefulWidget {
  const YTMusicSettingsScreen({super.key, this.embeddedInTab = false});

  /// When true, the screen is rendered as a bottom-navigation tab instead of a
  /// pushed route, so the leading back button is hidden.
...
class _YTMusicSettingsScreenState extends ConsumerState<YTMusicSettingsScreen> {
  ...
  // Artwork colors are reserved for Now Playing / mini player; settings stay
  // on the user's chosen accent.
  AlbumColors get _albumColors => AlbumColors.defaultColors();
  ...
  /// Screen background for the rendered body. When embedded as a bottom-nav
  /// tab it is transparent so the shell's gradient shows through (matching
  /// Home / Songs / Library). Bottom sheets keep the solid [_backgroundColor].
  Color get _screenBackgroundColor =>
      widget.embeddedInTab ? Colors.transparent : _backgroundColor;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: authState.isLoading
          ...
          : CustomScrollView(
              slivers: [
                // ── Header (matches Home / Songs / Library) ─────────────
                SliverAppBar(
                  backgroundColor: _screenBackgroundColor,
                  ...
                  titleSpacing: widget.embeddedInTab ? 16 : 0,
                  title: Text(
                    l10n.settings,
                    style: TextStyle(
                      fontSize: 28,
                      ...
                      color:
                          _isDark ? Colors.white : InzxColors.textPrimary,
                    ),
                  ),
                ),
                      ...
                      children: [
                        Expanded(
                          child: OutlinedButton.icon(
                            onPressed: () {
                              launchUrl(
                                Uri.parse('https://github.com/Anuragmagar/tuneload_v2'),

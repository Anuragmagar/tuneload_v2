...
/// Resolves the album name to display in Now Playing. Reads the tag directly
/// from the local file for tracks missing an album name (e.g. older downloads
/// built from a folder scan that dropped the album), matching what other music
/// players show for the same file.
final nowPlayingAlbumProvider =
    FutureProvider.family<String?, Track>((ref, track) async {
      final cached = track.album?.trim();
      if (cached != null && cached.isNotEmpty) return cached;

      final path = track.localFilePath?.trim();
      if (path == null || path.isEmpty) return null;

      try {
        final album = await compute(_readAlbumTagIsolate, path);
...
/// Reads the album tag from a local audio file (runs in an isolate via
/// [compute] so the File/parsing work never blocks the UI thread).
String? _readAlbumTagIsolate(String path) {
  try {
...
class _NowPlayingScreenState extends ConsumerState<NowPlayingScreen>
  ...
  static const double _syncedLyricPreviewHeight = 52;
  // Breathing room between the album art and the track title when there is no
  // synced lyric line to occupy that space (plain/no lyrics).
  static const double _noSyncedLyricGap = 20;
  ...
  @override
  void dispose() {
    // This screen is the only place that recolors the system navigation bar
    // (to match the album gradient). The app default is asserted imperatively
    // in main.dart, which doesn't re-run when a route pops, so re-assert it
    // here to keep the album color from lingering after leaving Now Playing.
    SystemChrome.setSystemUIOverlayStyle(
      const SystemUiOverlayStyle(
        statusBarColor: Colors.transparent,
        statusBarIconBrightness: Brightness.light,
        statusBarBrightness: Brightness.dark,
        systemNavigationBarColor: InzxColors.darkBottomNavigation,
  ...
  Widget build(BuildContext context) {
    ...
    final builtWidget = playbackState.when(
      data: (state) {
    ...
    // Match the system navigation bar with the bottom-most background that the
    // UP NEXT / LYRICS / RELATED rows sit on (bottom of the album gradient).
    final navColor = colors.backgroundSecondary;
    final navIconsDark = navColor.computeLuminance() > 0.5;
    final statusIconsDark =
        colors.backgroundPrimary.computeLuminance() > 0.5;
    final systemOverlay = SystemUiOverlayStyle(
      statusBarColor: Colors.transparent,
      statusBarIconBrightness:
          statusIconsDark ? Brightness.dark : Brightness.light,
      systemNavigationBarColor: navColor,
      systemNavigationBarIconBrightness:
          navIconsDark ? Brightness.dark : Brightness.light,
    );

    return PopScope(
      canPop: !_isDrawerExpanded,
      onPopInvokedWithResult: (didPop, result) {
        if (!didPop && _isDrawerExpanded) {
          _drawerKey.collapse();
        }
      },
  ...
  Widget _buildFullAlbumView(
    ...
    return Column(
      children: [
        // Top bar
        _buildTopBar(track, textColor, secondaryTextColor),
  ...
  Widget _buildTopBar(Track track, Color textColor, Color secondaryColor) {
    final playbackState = ref.watch(playbackStateProvider).valueOrNull;
    final queueTitle = playbackState?.queueTitle;

    final trackAlbum = track.album?.trim();
    final hasTrackAlbum = trackAlbum != null && trackAlbum.isNotEmpty;
    // When the track already carries an album use it directly (no tag read);
    // only fall back to reading the local file's tags for older downloads that
    // lost the album during a folder scan.
    final albumName = hasTrackAlbum
        ? trackAlbum
        : ref.watch(nowPlayingAlbumProvider(track)).valueOrNull;
    final hasAlbumName = albumName != null && albumName.isNotEmpty;
    // Show album name under "NOW PLAYING"; fall back to the queue title.
    final subtitle = hasAlbumName ? albumName : queueTitle?.trim();
  ...
  Widget _buildAlbumArt(Track track, Color accentColor) {
    ...
    // Keep album art at a fixed size regardless of lyrics availability so the
    // artwork never shrinks when synced lyrics appear/disappear.
    return LayoutBuilder(
      builder: (context, constraints) {
        final screenWidth = MediaQuery.of(context).size.width;
        final screenHeight = MediaQuery.of(context).size.height;

        // Fixed max-height ratio (doesn't change when lyrics appear)
        final maxHeightRatio = 0.47;
  ...
  Widget _buildAlbumArtContent(
    ...
    if (localAudioPath != null && localAudioPath.isNotEmpty) {
      // Privatized cover art lives in app-private storage; keyed by track id.
      final coverPath =
          ref.watch(downloadedCoverArtProvider(displayTrack!.id)).valueOrNull;
      if (coverPath != null && coverPath.isNotEmpty) {
        final localCoverFile = File(coverPath);
...
child: ClipRRect(
                  borderRadius: BorderRadius.circular(16),
                  child: _buildAlbumArtContent(ref, track, accentColor),
                ),
            ),
            _buildHeartOverlay(),
          ],
  ...
  Widget _buildSyncedLyricPreview(Color textColor) {
    ...
    final text = (currentIdx >= 0 && currentIdx < lines.length)
        ...
        : '';
    final bool showLyricLine = hasSynced && text.isNotEmpty;
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        duration: const Duration(milliseconds: 350),
        curve: Curves.easeOutCubic,
        height: showLyricLine ? _syncedLyricPreviewHeight : _noSyncedLyricGap,
        child: ClipRect(
          child: showLyricLine
              ? Padding(
                ...
                Consumer(
                  builder: (context, ref, child) {
                    ...
                    final phase = ref.watch(
                      trackDownloadPhaseProvider(track.id),
                    );
                    final isConverting = phase == DownloadPhase.converting;
                    ...
                    } else if (progress != null || isConverting) {
                    ...
                    return BouncyTouch(
                      ...
                      onTap: isDownloaded
                          ? () {
                              // Already downloaded - let it stay; no-op.
                            }
                          : progress == null && !isConverting
                              ? () {
                                  ref
                      ...
                      child: Padding(
                        ...
                        child: (progress != null && !isDownloaded) ||
                                isConverting
                            ? SizedBox(
                                width: 18,
                                height: 18,

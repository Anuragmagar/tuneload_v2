...
class AudioPlayerService {
  ...
  // Track ids we've already attempted background album resolution on.
  final Set<String> _albumLookupAttempted = {};
  ...
  Future<void> _loadAndPlayCurrent() async {
    ...
    // Best-effort album resolution for tracks that don't carry it (radio /
    // watch-playlist rows that auto-advance after the tapped song rarely do).
    // Search-result rows usually already have the album; this guarantees the
    // NOW PLAYING subtitle still gets one for every streamed track. Runs in
    // parallel with stream setup and never blocks playback.
    final rawAlbum = _currentTrack?.album?.trim();
    if (_currentTrack != null &&
        _currentTrack!.localFilePath == null &&
        _innerTubeService != null &&
        (rawAlbum == null || rawAlbum.isEmpty)) {
      final lookupId = _currentTrack!.id;
      if (!_albumLookupAttempted.contains(lookupId)) {
        _albumLookupAttempted.add(lookupId);
        unawaited(_resolveCurrentTrackAlbumInBackground(lookupId));
      }
    }
  ...
  Future<void> _fetchRadioTracks() async {
    ...
    try {
      ...
      Track? currentEnrichment;
      final radioTracks = await _innerTubeService!.getWatchPlaylist(
        sourceId,
        limit: 25, // Request more for better selection
        onCurrentTrack: (current) {
          currentEnrichment = current;
        },
      );

      // Enrich the source/current track with album metadata from the watch
      // response (single-song/radio plays rarely carry it in the row data).
      if (currentEnrichment != null &&
          _currentTrack != null &&
          _currentTrack!.id == sourceId &&
          _currentTrack!.localFilePath == null) {
        final album = currentEnrichment!.album;
        if (album != null &&
            album.trim().isNotEmpty &&
            (_currentTrack!.album == null ||
                _currentTrack!.album!.trim().isEmpty)) {
          final updated = _currentTrack!.copyWith(
            album: album.trim(),
            albumId: currentEnrichment!.albumId,
          );
          _currentTrack = updated;
          if (_currentIndex >= 0 &&
              _currentIndex < _queue.length &&
              _currentIndex < _originalQueue.length) {
            _queue[_currentIndex] = updated;
            _originalQueue[_currentIndex] = updated;
            _queueRevision++;
            _updateState(
      ...
      // Best-effort: the watch response often has no album for official music
      // videos / remixes. Resolve it via a songs-filtered search in the
      // background so the NOW PLAYING subtitle can still get the album.
      // Fire-and-forget; never blocks playback.
      final currentAlbumAgain = _currentTrack?.album?.trim();
      if (_currentTrack != null &&
          _currentTrack!.localFilePath == null &&
          (currentAlbumAgain == null || currentAlbumAgain.isEmpty)) {
        final lookupId = _currentTrack!.id;
        if (!_albumLookupAttempted.contains(lookupId)) {
          _albumLookupAttempted.add(lookupId);
          unawaited(_resolveCurrentTrackAlbumInBackground(lookupId));
  ...
  /// Background album resolution for a track the watch response couldn't
  /// enrich. Patches the current track and its queue entry if a value is
  /// found while the same track is still playing.
  Future<void> _resolveCurrentTrackAlbumInBackground(String videoId) async {
    final snapshot = _currentTrack;
    if (snapshot == null || snapshot.localFilePath != null) return;

    final result =
        await _innerTubeService!.resolveAlbumForTrack(snapshot);
    final album = result.album;
    if (album == null || album.trim().isEmpty) return;

    if (_currentTrack == null ||
        _currentTrack!.id != videoId ||
        _currentTrack!.localFilePath != null) {
      return;
    }
    final currentAlbum = _currentTrack!.album?.trim();
    if (currentAlbum != null && currentAlbum.isNotEmpty) return;

    final updated = _currentTrack!
        .copyWith(album: album.trim(), albumId: result.albumId);
    _currentTrack = updated;
    if (_currentIndex >= 0 &&
        _currentIndex < _queue.length &&
        _currentIndex < _originalQueue.length) {

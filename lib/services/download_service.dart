...
/// Get a private directory for cover art that never gets into the OS gallery.
/// Cover art lives in app documents (not the public Downloads folder) so it is
/// never indexed into the gallery/photo apps.
...
/// Deterministic cover art path for a track (app-private, not gallery-scanned).
Future<String> _coverPathFor(String trackId) async {
  final dir = await _getCoverDir();
  return '${dir.path}/$trackId.jpg';
}

/// Absolute path of a downloaded track's cover art if it exists (null otherwise).
final downloadedCoverArtProvider =
    FutureProvider.family<String?, String>((ref, trackId) async {
      if (trackId.trim().isEmpty) return null;
      try {
        final file = File(await _coverPathFor(trackId));
...
/// Sub-phase of a running download used to show an indeterminate progress bar
/// while the downloaded file is being converted/tagged with FFmpeg.
enum DownloadPhase { downloading, converting }
...
class DownloadTask {
  ...
  final DownloadPhase phase; // Progress sub-phase (indeterminate while converting)
  ...
  bool get isConverting => status == DownloadStatus.downloading && phase == DownloadPhase.converting;
...
class DownloadManagerNotifier extends StateNotifier<DownloadManagerState> {
  ...
  /// success, or null to keep the original, along with the resolved album
  /// (used to persist richer metadata even when conversion is skipped/fails).
  Future<({String? path, String? album})> _convertToM4a(
    DownloadTask task,
    String sourcePath,
  ) async {
    ...
    try {
      ...
      if (dotIndex <= 0) return (path: null, album: meta.album);
      final outputPath = '${sourcePath.substring(0, dotIndex)}.m4a';

      ActivityLogService.i('convert', 'Converting "${task.track.title}" to M4A/AAC...');

      // Cover lives in app-private storage (never in the OS gallery).
      final coverPath = await _coverPathFor(task.trackId);
      ...
      if (!ReturnCode.isSuccess(returnCode)) {
        ...
        return (path: null, album: meta.album);
      }
      ...
      return (path: outputPath, album: meta.album);
    } catch (e) {
      if (kDebugMode) {
  ...
  Future<void> removeDownload(String trackId) async {
    ...
    if (task.localPath != null) {
      try {
        ...
        // Legacy cover files that lived next to the audio in the public
        // downloads folder (polluted the gallery); clean them up.
        final legacyCoverFile = File('${task.localPath!}.cover.jpg');
        if (await legacyCoverFile.exists()) {
          await legacyCoverFile.delete();
        }
      } catch (e) {
        if (kDebugMode) {
      ...
      // Delete the app-private cover art (now playing artwork).
      try {
        final coverFile = File(await _coverPathFor(trackId));
  ...
  Future<void> _downloadCoverArtForTrack({
    ...
    final tried = <String>{};
    // App-private storage, never in the OS gallery.
    final coverFile = File(await _coverPathFor(trackId));
  ...
  Future<void> _downloadTrack(DownloadTask task) async {
    ...
    try {
      ...
      // Save cover art in app-private storage (never indexable by the gallery).
      ...
      // support (title, artist, album and cover art). The conversion phase
      // shows an indeterminate progress bar in the UI/notification.
      // Keep the task in the "downloading" status with a final progress value
      // while converting, so the progress/phase providers keep reporting an
      // active task (a stale queued copy would make UIs fall back to idles).
      _updateTask(
        task.trackId,
        task.copyWith(
          status: DownloadStatus.downloading,
      ...
      await _notificationService.showDownloadConverting(
        task.trackId,
        task.track.title,
      );
      final converted = await _convertToM4a(task, filePath);
      final finalPath = converted.path ?? filePath;
      final resolvedAlbum = converted.album;

      // Make the file discoverable by other music apps on Android.
      await _notifyMediaScanner(finalPath);

      // Persist the album name resolved at conversion time (e.g. from the
      // YouTube Music API) so the downloaded track keeps it across reboots.
      final hasResolvedAlbum =
          resolvedAlbum?.trim().isNotEmpty == true &&
          task.track.album?.trim().isNotEmpty != true;
      final enrichedTrack = hasResolvedAlbum
          ? task.track.copyWith(album: resolvedAlbum!.trim())
          : task.track;
      ...
      final persistedTask = hasResolvedAlbum
          ? DownloadTask(
              trackId: completedTask.trackId,
              track: enrichedTrack,
              status: completedTask.status,
              phase: completedTask.phase,
              progress: completedTask.progress,
              downloadedBytes: completedTask.downloadedBytes,
              totalBytes: completedTask.totalBytes,
              error: completedTask.error,
              localPath: completedTask.localPath,
              startedAt: completedTask.startedAt,
            )
          : completedTask;
      _updateTask(task.trackId, persistedTask);
      ...
      await _persistDownload(persistedTask);
      ...
      try {
        ...
        final privateCover = File(await _coverPathFor(task.trackId));
        if (await privateCover.exists()) {
          await privateCover.delete();
        }
      } catch (_) {}
...
/// Provider for the current download phase of a specific track (used to show
/// an indeterminate progress indicator while the file is converting).
final trackDownloadPhaseProvider = Provider.family<DownloadPhase?, String>((
  ref,
  trackId,
) {
  final downloadState = ref.watch(downloadManagerProvider);
